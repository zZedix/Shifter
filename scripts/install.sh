...
maybe_configure_https() {
    ...
    if [[ -n "${SHIFTER_ENABLE_HTTPS:-}" ]]; then
        ...
        while true; do
            printf "Do you want to enable HTTPS? [y/N] " >"${INSTALLER_TTY}" 2>/dev/null || true
            if ! read -r answer <"${INSTALLER_TTY}" 2>/dev/null; then
                answer=""
            fi
    ...
    case "${choice}" in
        y|yes)
            ...
            if [[ -n "${SHIFTER_DOMAIN:-}" ]]; then
                ...
                printf "Domain name: " >"${INSTALLER_TTY}" 2>/dev/null || true
                if ! read -r CERT_DOMAIN <"${INSTALLER_TTY}" 2>/dev/null; then
                    CERT_DOMAIN=""
                fi
            else
            ...
            if [[ -n "${SHIFTER_CONTACT_EMAIL:-}" ]]; then
                ...
                printf "Contact email for Let's Encrypt: " >"${INSTALLER_TTY}" 2>/dev/null || true
                if ! read -r CERT_EMAIL <"${INSTALLER_TTY}" 2>/dev/null; then
                    CERT_EMAIL=""
                fi
            else
...
Run this to create WebUI credentials:
  sudo SHIFTER_AUTH_FILE='${AUTH_FILE}' shifter-toolkit reset-credentials --generate
EOF
...
Next steps:
  1. sudo SHIFTER_AUTH_FILE='${AUTH_FILE}' shifter-toolkit reset-credentials --generate
